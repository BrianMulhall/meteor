if (process.env.ROOT_URL && typeof __meteor_runtime_config__ === "object") {
  
  __meteor_runtime_config__.ROOT_URL = process.env.ROOT_URL;

  if (__meteor_runtime_config__.ROOT_URL) {
    
    const parsedUrl = Npm.require('url').parse(__meteor_runtime_config__.ROOT_URL);
    ...
    let pathPrefix = parsedUrl.pathname;
